...
function listSkills(cwd) {
  if (cwd) skillCwd = cwd;
  const priority = { project: 4, user: 3, "user-commands": 3, bundled: 2, compat: 1 };
  const map = new Map();
  const roots = skillRoots().slice();
  const proj = skillCwd ? require("path").join(skillCwd, ".grok", "skills") : null;
  if (proj && require("fs").existsSync(proj)) roots.push({ scope: "project", dir: proj });
...
function createSkill({ name, description }) {
  const safe = String(name || "")
    ...
    .replace(/[^a-z0-9-_]+/g, "-")
    .replace(/^-+|-+$/g, "");
  if (!safe) throw new Error("名称无效：请用英文、数字和短横线，例如 code-review");
