...
| 项 | 说明 |
|------|------|
| 思考残片 | 官方 CLI 在转入工具调用时可能以半句、半个 URL 或空列表项结束思考；现在会去掉空编号并用省略号明确标记，而不再看起来像页面裁掉了最后一行 |
| 真实内容 | 不编造上游从未发送的后半句；实时与历史页面使用同一封口规则 |
...
| 回归测试 | 加入未闭合思考、路径规范化、聚合状态及折叠区归属样例，逻辑测试增至 38 个 |
